...
class AccountReportManager(models.Model):
    ...
    summary = fields.Char()
    footnotes_ids = fields.One2many('ac.report.footnote', 'manager_id')
    company_id = fields.Many2one('res.company')
    financial_report_id = fields.Many2one('account.financial.html.report')

    def add_footnote(self, text, line):
        return self.env['ac.report.footnote'].create({'line': line, 'text': text, 'manager_id': self.id})

class AccountReportFootnote(models.Model):
    _name = 'ac.report.footnote'
    _description = 'Footnote for reports'

    text = fields.Char()
    line = fields.Char(index=True)
    manager_id = fields.Many2one('ac.report.manager')
...
class AccountReport(models.AbstractModel):
    ...
    filter_analytic2 = None
    filter_partner = None
    
    filter_show_partner = None
    filter_show_currency = None
    
    c_account_ids = []
    c_account_tag_ids = []
    c_account_analytic_ids = []
    c_account_analytic2_ids = []
    c_partner_ids = []
    ...
    def get_templates(self):
        return {
                'main_template': 'ac_reports.main_template',
                'line_template': 'ac_reports.line_template',
                'footnotes_template': 'ac_reports.footnotes_template',
                'search_template': 'ac_reports.search_template',
        }
    ...
    def open_general_ledger(self, options, params=None):
        ...
        ctx = self.env.context.copy()
        ctx.pop('id', '')
        action = self.env.ref('ac_reports.action_ac_report_general_ledger').read()[0]
        options['unfolded_lines'] = ['account_%s' % (params.get('id', ''),)]
        if params.get('id', False):
            options['account_id'] = [params.get('id')]
        options['opt_current'] = True    
        options['unfold_all'] = False
    ...
    def get_report_informations(self, options):
        ...
        '''
        opt = options or {}
        self.searchview_dict = {}
        opt_current = opt.get('opt_current', False)
        
        if opt.get('partner_id') and type(opt.get('partner_id')) != list:
            opt['partner_id'] = [opt.get('partner_id')]
        
        self.c_account_ids = opt.get('account_id') and [int(x) for x in opt.get('account_id')] or (opt_current and opt.get('c_account_ids', [])) or []
        self.c_account_tag_ids = opt.get('account_tag_id') and [int(x) for x in opt.get('account_tag_id')] or (opt_current and opt.get('c_account_tag_ids', [])) or []
        self.c_account_analytic_ids = opt.get('account_analytic_id') and [int(x) for x in opt.get('account_analytic_id')] or (opt_current and opt.get('c_account_analytic_ids', [])) or []
        self.c_account_analytic2_ids = opt.get('account_analytic2_id') and [int(x) for x in opt.get('account_analytic2_id')] or (opt_current and opt.get('c_account_analytic2_ids', [])) or []
        self.c_partner_ids = opt.get('partner_id') and [int(x) for x in opt.get('partner_id')] or (opt_current and opt.get('c_partner_ids', [])) or []
        ...
        if len(self.c_account_analytic_ids) > 0:
            self.searchview_dict['account_analytic_id'] = [(t.id, t.name) for t in self.env['account.analytic.account'].browse(self.c_account_analytic_ids)]
        
        if len(self.c_account_analytic2_ids) > 0:
            self.searchview_dict['account_analytic2_id'] = [(t.id, t.name) for t in self.env['account.analytic.account'].browse(self.c_account_analytic2_ids)]
        
        if len(self.c_account_ids) > 0:
            self.searchview_dict['account_id'] = [(t.id, t.name) for t in self.env['account.account'].browse(self.c_account_ids)]
        
        if len(self.c_account_tag_ids) > 0:
            self.searchview_dict['account_tag_id'] = [(t.id, t.name) for t in self.env['account.account.tag'].browse(self.c_account_tag_ids)]
        
        if len(self.c_partner_ids) > 0:
            self.searchview_dict['partner_id'] = [(t.id, t.name) for t in self.env['res.partner'].browse(self.c_partner_ids)]
        
        searchview_dict.update(self.searchview_dict)
        
        options.update({'c_account_analytic_ids': self.c_account_analytic_ids,
                        'c_account_analytic2_ids': self.c_account_analytic2_ids,
                        'c_account_ids': self.c_account_ids,
                        'c_account_tag_ids': self.c_account_tag_ids,
                        'c_partner_ids': self.c_partner_ids})
        ...
        info = {'options': options,
                ...
                'main_html': self.get_html(options,report_manager=report_manager),
                'searchview_html': self.env['ir.ui.view'].render_template(self.get_templates().get('search_template', 'ac_report.search_template'), values=searchview_dict),
                }
        #for k in ['c_account_analytic_ids', 'c_account_analytic2_ids', 'c_account_ids', 'c_account_tag_ids', 'c_partner_ids']:
        #    del info['options'][k]
    ...
    @api.multi
    def get_html(self, options, line_id=None, additional_context=None, report_manager=None):
        '''
        ...
        * if line_id is set, the template used will be the line_template
        ...
        report_manager = report_manager or self.get_report_manager(options)
        ...
        render_template = templates.get('main_template', 'ac_reports.main_template')
        if line_id is not None:
            render_template = templates.get('line_template', 'ac_reports.line_template')
        ...
        if self.env.context.get('print_mode', False):
            ...
            html = html.replace(b'<div class="js_ac_report_footnotes"></div>', self.get_html_footnotes(footnotes_to_render))
        return html

    @api.multi
    def get_html_footnotes(self, footnotes):
        template = self.get_templates().get('footnotes_template', 'ac_reports.footnotes_template')
    ...
    def get_report_manager(self, options):
        ...
        existing_manager = self.env['ac.report.manager'].search(domain, limit=1)
        if not existing_manager:
            existing_manager = self.env['ac.report.manager'].create({'report_name': self._name, 'company_id': selected_companies and selected_companies[0] or False, 'financial_report_id': self.id if 'id' in dir(self) else False})
    ...
    def print_pdf(self, options):
        return {
                'type': 'ir_actions_ac_report_download',
    ...
    def replace_class(self):
        ...
        """
        return {b'o_ac_reports_no_print': b'', b'table-responsive': b'', b'<a': b'<span', b'</a>': b'</span>'}
    ...
    def get_pdf(self, options, minimal_layout=True):
        ...
        body = self.env['ir.ui.view'].render_template(
            "ac_reports.print_template",
            values=dict(rcontext),
        )
        body_html = self.with_context(print_mode=True).get_html(options)

        body = body.replace(b'<body class="o_ac_reports_body_print">', b'<body class="o_ac_reports_body_print">' + body_html)
        if minimal_layout:
        ...
        landscape = False
        if len(self.with_context(print_mode=True).get_columns_name(options)) > 7:
            landscape = True
    ...
    def get_model_data(self, options, additional_context=None):
        model = False
        name = options.get('q') or ''
        domain = []
        filter = options.get('filter')
        if filter == 'account_id' :
            model = 'account.account'
        elif filter == 'account_tag_id' :
            model = 'account.account.tag'
        elif filter == 'account_analytic_id' :
            model = 'account.analytic.account'
            domain = [('is_truck', '=', False)]
        elif filter == 'account_analytic2_id' :
            model = 'account.analytic.account'
            domain = [('is_truck', '=', True)]
        elif filter == 'partner_id' :
            model = 'res.partner'
            domain = ['|', ('is_company', '=', True), ('is_trans', '!=', True)]
        else:
            return []
        
        if options.get('val'): 
            if type(options.get('val')) is list:
                domain.append(('id', 'not in', options.get('val')))
            else:
                domain.append(('id', '!=', options.get('val')))
        
        
        
        return self.env[model].name_search(name, domain, limit=20)
        
        
    def sql_join(self, op, filed, val):
        s = ''
        if val:
            if type(val) == list:
                if len(val) > 1:
                    val = tuple(val).__str__()
                    val = val.replace(",)", ")")
                    return " %s %s IN %s" % (op, filed, val)
                else:
                    val = val[0]
            return " %s %s = %s" % (op, filed, val)
        return ''
    
    def index_rem(self, indices, arra): 
