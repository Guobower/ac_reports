...
class report_account_followup_report(models.AbstractModel):
    ...
    def get_report_manager(self, options):
        ...
        if not existing_manager:
            existing_manager = self.env['ac.report.manager'].create({'report_name': 'account.followup.report', 
    ...
    def get_templates(self):
        ...
        templates['main_template'] = 'ac_reports.template_followup_report'
        templates['line_template'] = 'ac_reports.line_template_followup_report'
        if self.env.context.get('print_mode') and not self.env.context.get('mail'):
            templates['main_template'] = 'ac_reports.report_followup_letter'
        return templates

    def get_history(self, partner):
        return self.env['mail.message'].search([('subtype_id', '=', self.env.ref('ac_reports.followup_logged_action').id), ('id', 'in', partner.message_ids.ids)], limit=5)
    ...
    def change_next_action(self, partner_id, date, note):
        ...
        partner.message_post(body=msg, subtype='ac_reports.followup_logged_action')
        return True
    ...
    def send_email(self, options):
        ...
        if email and email.strip():
            ...
            msg_id = partner.message_post(body=msg, subtype='ac_reports.followup_logged_action')
...
class ac_report_followup_all(models.AbstractModel):
    ...
    def get_options(self, previous_options):
        options = super(ac_report_followup_all, self).get_options(previous_options)
    ...
    def get_templates(self):
        ...
        if self.env.context.get('print_mode') and not self.env.context.get('mail'):
            ...
            templates['main_template'] = 'ac_reports.report_followup_all'
            
        templates['line_template'] = 'ac_reports.line_template_followup_report'
        templates['search_template'] = 'ac_reports.followup_search_template'
        return templates

    def get_lines(self, options, line_id=None):
        lines = []
        if self.env.context.get('print_mode'):
            return super(ac_report_followup_all, self).get_lines(options, line_id=line_id)
        for partner_id in options.get('partners_to_show'):
            options['partner_id'] = partner_id
            lines.append(super(ac_report_followup_all, self).get_lines(options, line_id=line_id))
        return lines
    ...
    def get_report_informations(self, options):
        informations = super(ac_report_followup_all, self).get_report_informations(options)
