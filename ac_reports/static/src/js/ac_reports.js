odoo.define('ac_reports.ac_report', function (require) {
'use strict';
...
var accountReportsWidget = Widget.extend(ControlPanelMixin, {
    ...
    events: {
        'click .o_ac_reports_summary': 'edit_summary',
        'click .js_ac_report_save_summary': 'save_summary',
        'click .o_ac_reports_footnote_icons': 'delete_footnote',
        'click .js_ac_reports_add_footnote': 'add_edit_footnote',
        'click .js_ac_report_foldable': 'fold_unfold',
    ...
    render_template: function() {
        ...
        this.$el.find('.o_ac_reports_summary_edit').hide();
    },
    render_searchview_buttons: function() {
        var self = this;
        // bind searchview buttons/filter to the correct actions
        var $datetimepickers = this.$searchview_buttons.find('.js_ac_reports_datetimepicker');
        ...
        _.each(this.$searchview_buttons.find('.js_ac_report_bool_filter'), function(k) {
            $(k).toggleClass('selected', self.report_options[$(k).data('filter')]);
        });
        _.each(this.$searchview_buttons.find('.js_ac_report_choice_filter'), function(k) {
        ...
        _.each(this.$searchview_buttons.find('.js_ac_reports_one_choice_filter'), function(k) {
            $(k).toggleClass('selected', ''+self.report_options[$(k).data('filter')] === ''+$(k).data('id'));
        });
        // click event
        this.$searchview_buttons.find('.js_ac_report_date_filter').click(function (event) {
        ...
        this.$searchview_buttons.find('.js_ac_report_bool_filter').click(function (event) {
        ...
        this.$searchview_buttons.find('.js_ac_report_choice_filter').click(function (event) {
            var option_value = $(this).data('filter');
        ...
        this.$searchview_buttons.find('.js_ac_reports_one_choice_filter').click(function (event) {
            self.report_options[$(this).data('filter')] = $(this).data('id');
            self.reload();
        });
        this.$searchview_buttons.find('.js_ac_report_date_cmp_filter').click(function (event){
        ...
        // analytic filter
        /*
        this.$searchview_buttons.find('.js_ac_reports_analytic_auto_complete').select2();
        ...
        this.$searchview_buttons.find('.js_ac_reports_analytic_auto_complete').on('change', function(){
    ...
    edit_summary: function(e) {
        var $textarea = $(e.target).parents('.o_ac_reports_body').find('textarea[name="summary"]');
        var height = Math.max($(e.target).parents('.o_ac_reports_body').find('.o_ac_report_summary').height(), 100); // Compute the height that will be needed
        ...
        $textarea.val(text);
        $(e.target).parents('.o_ac_reports_body').find('.o_ac_reports_summary_edit').show();
        $(e.target).parents('.o_ac_reports_body').find('.o_ac_reports_summary').hide();
        $(e.target).parents('.o_ac_reports_body').find('textarea[name="summary"]').focus();
    },
    save_summary: function(e) {
        var self = this;
        var text = $(e.target).siblings().val().replace(/[ \t]+/g, ' ');
        return this._rpc({
                model: 'ac.report.manager',
                method: 'write',
                args: [this.report_manager_id, {summary: text}],
                context: this.odoo_context,
            })
            .then(function(result){
                self.$el.find('.o_ac_reports_summary_edit').hide();
                self.$el.find('.o_ac_reports_summary').show();
                if (!text) {
                    text = "<input type='text' class='o_input o_field_widget' name='summary' placeholder='Click to add an introductory explanation' />";
                }
                return $(e.target).parent().siblings('.o_ac_reports_summary').find('> .o_ac_report_summary').html('<span>'+text+'</span');
            });
    },
    render_footnotes: function() {
        var self = this;
        // First assign number based on visible lines
        var $dom_footnotes = self.$el.find('.js_ac_report_line_footnote:not(.folded)');
        $dom_footnotes.html('');
        var number = 1;
        ...
        _.each($dom_footnotes, function(el) {
            ...
            if (footnote.length !== 0) {
                $(el).html('<sup><b class="o_ac_reports_footnote_sup"><a href="#footnote'+number+'">'+number+'</a></b></sup>');
        ...
        return this._rpc({
            ...
            .then(function(result){
                return self.$el.find('.js_ac_report_footnotes').html(result);
            });
    },
    add_edit_footnote: function(e) {
        ...
        var save = function() {
            var footnote_text = $('.js_ac_reports_footnote_note').val().replace(/[ \t]+/g, ' ');
            if (!footnote_text && existing_footnote.length === 0) {return;}
            if (existing_footnote.length !== 0) {
                if (!footnote_text) {
                    return self.$el.find('.footnote[data-id="'+existing_footnote[0].id+'"] .o_ac_reports_footnote_icons').click();
                }
                // replace text of existing footnote
                return this._rpc({
                        model: 'ac.report.footnote',
                        method: 'write',
    ...
    fold_unfold: function(e) {
        var self = this;
        if ($(e.target).hasClass('caret') || $(e.target).parents('.o_ac_reports_footnote_sup').length > 0){return;}
        e.stopPropagation();
        e.preventDefault();
    ...
    fold: function(line) {
        ...
        if ($lines_to_hide.length > 0) {
            ...
            $lines_to_hide.find('.js_ac_report_line_footnote').addClass('folded');
            $lines_to_hide.hide();
    ...
    unfold: function(line) {
        ...
        if ($lines_in_dom.length > 0) {
            $lines_in_dom.find('.js_ac_report_line_footnote').removeClass('folded');
            $lines_in_dom.show();
    ...
    unfold_all: function(bool) {
        var self = this;
        var lines = this.$el.find('.js_ac_report_foldable');
    ...
    render_select2 : function(){
        window.render_select2 = this.render_select2;
        var self = this, a = self.$searchview_buttons.find('.dropdown-toggle.account_analytic_filter');
        var mode_select2 = self.$searchview_buttons.find('.model_select2');
        _.each(mode_select2, function(el){
                el = $(el)
                var data = el.data(), parent = el.parent(), filter = el.data('filter');
                el.select2({
                  ...
                  ajax: {
                    ...
                    transport: function (params, success, failure) {
                        ...
                        if (params.data) 
                            query['q'] = params.data.q;
                        if (el.val()) 
                            query['val'] = el.val();
                        self._rpc({
                            model: self.report_model,
                            method: 'get_model_data',
                            args: [self.financial_id, query],
                            context: self.odoo_context,
                        })
                        .then(function(data){
                            ...
                            for(var i=0; i < data.length; i++){
                                var d = data[i];
                                r.push({id: d[0], text: d[1]});
                            }
                            //console.log("result2:", {result:r});
                            success({results:r});
                        });
                    },
        ...
        self.$searchview_buttons.find('.select2.select2-container').css({'min-width': '180px'});
        this.$searchview_buttons.find('.js_account_select_button_filter').click(function (event) {
            self.reload();
        });
        mode_select2.on('change', function(e){
            setTimeout( function(){
    ...
    fill_other_option: function(){
        var self = this;
        var mode_select2 = self.$searchview_buttons.find('.model_select2');
        console.log("fill_other_option");
        _.each(mode_select2, function(el){
            el = $(el)
            //console.log(el.data('filter'), ":", el.val());
            self.report_options[el.data('filter')] = el.val();
        });
...
ActionManager.include({
    ir_actions_ac_report_download: function(action, options) {
        var self = this;
        var c = crash_manager;
...
core.action_registry.add('ac_report', accountReportsWidget);
