odoo.define('ac_reports.ac_report_followup', function (require) {
'use strict';
...
var ac_report = require('ac_reports.ac_report');

var QWeb = core.qweb;

var ac_report_followup = ac_report.extend({
    events: _.defaults({
        ...
        'click .o_ac_reports_followup_skip': 'skip_partner',
        'click .o_ac_reports_followup_done': 'done_partner',
        'click .o_ac_reports_followup-auto': 'enable_auto',
        "change *[name='blocked']": 'on_change_blocked',
        'click .o_ac_reports_followup-set-next-action': 'set_next_action',
    }, ac_report.prototype.events),
    ...
    display_done: function(e) {
        $(e.target).parents('.o_ac_reports_body').find("div.o_ac_reports_page").find(".alert.alert-info.alert-dismissible").remove();
        $(e.target).parents('.o_ac_reports_body').find('#action-buttons').addClass('o_ac_reports_followup_clicked');
    ...
    send_followup_email: function(e) {
        ...
        return this._rpc({
            ...
            .then(function (result) { // send the email server side
                self.display_done(e);
                $(e.target).parents("div.o_ac_reports_page").prepend(QWeb.render("emailSent")); // If all went well, notify the user
            });
    },
    ...
    set_next_action: function(e) {
        ...
        nextActionDatePicker.appendTo($content.find('div.o_ac_reports_next_action_date_picker'));
        ...
        $content.find('.o_ac_reports_followup_next_action_date_button').bind('click', changeDate);

        var save = function () {
            var note = $content.find(".o_ac_reports_next_action_note").val().replace(/\r?\n/g, '<br />').replace(/\s+/g, ' ');
            var date = nextActionDatePicker.getValue();
            var target_id = $content.find("#target_id").val();
            if (self.$el.find('.o_ac_reports_followup-manual').hasClass('btn-default')){
    ...
    enable_auto: function(e) {
        ...
        if ($(e.target).parents('.o_ac_reports_body').find('.o_ac_reports_followup-auto:last').hasClass('btn-default')){
    ...
    toggle_auto_manual: function(e) {
        $(e.target).parents('.o_ac_reports_body').find('.o_ac_reports_followup-manual').toggleClass('btn-default btn-info'); // Change the highlighted buttons
        $(e.target).parents('.o_ac_reports_body').find('.o_ac_reports_followup-auto:last').toggleClass('btn-default btn-info'); // Change the highlighted buttons
    },
    display_exp_note_modal: function(e) {
        ...
        paymentDatePicker.appendTo($content.find('div.o_ac_reports_payment_date_picker'));
        var save = function () {
...
core.action_registry.add("ac_report_followup", ac_report_followup);
return ac_report_followup;
});
