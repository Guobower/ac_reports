...
class report_account_general_ledger(models.AbstractModel):
    ...
    filter_analytic2 = True
    filter_partner = True
    filter_account = True
    filter_account_tag = True
    
    filter_show_partner = False
    filter_show_currency = False
    ...
    def get_templates(self):
        ...
        templates['main_template'] = 'ac_reports.template_general_ledger_report'
        templates['line_template'] = 'ac_reports.line_template_general_ledger_report'
        return templates
    ...
    def get_columns_name(self, options):
        ...
        if options.get('show_partner'):
            cols.insert(3, {'name': _("Partner")})
            if options.get('show_currency'):
                cols.insert(4, {'name': _("Currency"), 'class': 'number'})
        elif options.get('show_currency'):
                cols.insert(3, {'name': _("Currency"), 'class': 'number'})
        if not options.get('show_partner'): 
            cols[2]['class'] = 'wrap_if'
        if options.get('is_single'):
            cols[0]['name'] = _("Journal Entry")
        return cols
    ...
    def _do_query(self, options, line_id, group_by_account=True, limit=False):
        ...
        if line_id:
            line_clause = self.sql_join('AND', '\"account_move_line\".account_id', line_id)
        
        else:
            line_clause = self.sql_join('AND', '\"account_move_line\".account_id', options.get('c_account_ids'))
        
        line_clause += self.sql_join('AND', '\"account_move_line\".analytic_account_id', options.get('c_account_analytic_ids'))
        line_clause += self.sql_join('AND', '\"account_move_line\".analytic_account2_id', options.get('c_account_analytic2_ids'))
        line_clause += self.sql_join('AND', '\"account_move_line\".account_tag_id', options.get('c_account_tag_ids'))
        line_clause += self.sql_join('AND', '\"account_move_line\".partner_id', options.get('c_partner_ids'))
        
        #raise ValidationError(line_clause)
    ...
    def group_by_account_id(self, options, line_id):
        ...
        if options.get('c_account_ids'):
            base_domain += [('account_id', 'in', options.get('c_account_ids'))]
        if options.get('c_account_analytic_ids'):
            base_domain += [('analytic_account_id', 'in', options.get('c_account_analytic_ids'))]
        if options.get('c_account_analytic2_ids'):
            base_domain += [('analytic_account2_id', 'in', options.get('c_account_analytic2_ids'))]
        if options.get('c_partner_ids'):
            base_domain += [('partner_id', 'in', options.get('c_partner_ids'))]
        if options.get('c_account_tag_ids'):
            base_domain += [('account_tag_id', 'in', options.get('c_account_tag_ids'))]
        ...
        for account_id, result in results.items():
            ...
            result['initial_bal'] = initial_bal_results.get(account.id, {'balance': 0, 'amount_currency': 0, 'debit': 0, 'credit': 0})
            if not unaffected_earnings_line and account.user_type_id.id == self.env.ref('account.data_unaffected_earnings').id :
                ...
                for field in ['balance', 'debit', 'credit']:
                    result['initial_bal'][field] += unaffected_earnings_results[field]
                    result[field] += unaffected_earnings_results[field]
            ...
            result['lines'] = self.env['account.move.line'].browse(aml_ids)
    ...
    def set_context(self, options):
        ctx = super(report_account_general_ledger, self).set_context(options)
        if options.get('show_partner'):
            ctx['show_partner'] = True
        if options.get('show_currency'):
            ctx['show_currency'] = True
        return ctx
        
    @api.model
    def get_options(self, options=None):
        self.is_single = len(self.c_account_ids) == 1
        opt = super(report_account_general_ledger, self).get_options(options)
        opt['is_single'] = self.is_single
        return opt
    ...
    def get_lines(self, options, line_id=None):
        ...
        to_rem = []
        name_len = 40
        single, show_partner, show_currency  = options.get('is_single', False),  options.get('show_partner', False), options.get('show_currency', False)
        ...
        for account in sorted_accounts:
            ...
            credit = row['credit']
            balance = row['balance']
            amount_currency = '' if not account.currency_id else self.format_value(row['amount_currency'], currency=account.currency_id)
            
            if not single:
                columns = show_currency and [amount_currency, self.format_value(debit), self.format_value(credit), self.format_value(balance)] or [self.format_value(debit), self.format_value(credit), self.format_value(balance)]
                lines.append({
            ...
            if single or 'account_%s' % (account.id,) in options.get('unfolded_lines') or unfold_all:
                initial_debit = row['initial_bal']['debit']
                initial_credit = row['initial_bal']['credit']
                initial_balance = row['initial_bal']['balance']
                initial_currency = '' if not account.currency_id else self.format_value(row['initial_bal']['amount_currency'], currency=account.currency_id)
                domain_lines = [{
                    'id': 'initial_%s' % (account.id,),
                    'class': 'o_ac_reports_initial_balance',
                ...
                for line in amls:
                    ...
                    if len(name) > name_len and not self.env.context.get('no_format'):
                        name = name[:name_len] + "..."
    ...
    def view_all_journal_items(self, options, params):
        ...
        return self.env['ac.report'].open_journal_items(options, params)
