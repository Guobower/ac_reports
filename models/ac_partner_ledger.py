...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def get_templates(self):
        ...
        templates['line_template'] = 'ac_reports.line_template_partner_ledger_report'
        templates['main_template'] = 'ac_reports.template_partner_ledger_report'
        return templates
    ...
    def do_query(self, options, line_id):
        ...
        if not account_types and not options.get('c_account_ids'):
        ...
        if line_id:
            line_clause = self.sql_join('AND', '\"account_move_line\".partner_id', line_id)
        else:
    ...
    def group_by_partner_id(self, options, line_id):
        ...
        #raise ValidationError("date_from: %s" % date_from)
    ...
    def get_lines(self, options, line_id=None):
        ...
        for partner in sorted_partners:
            ...
            if 'partner_' + str(partner.id) in options.get('unfolded_lines') or unfold_all:
                ...
                too_many = False
                if len(amls) > 1000 and not context.get('print_mode'):
                    amls = amls[-1000:]
                    too_many = True
                for line in amls:
                    if options.get('cash_basis'):
